/**
 * scripts/test-e2e-nile.mjs
 *
 * Runs the complete end-to-end WDK flow on Nile testnet:
 * 1. POST /api/intent with { "text": "Reserve 20 USDt for tickets" }
 * 2. POST /api/confirm/:id to sign offline
 * 3. POST /api/broadcast/:id to broadcast to Nile
 * 4. GET /api/wallet/balance to check new balance
 */

const API_BASE = "http://localhost:3001";

async function run() {
  console.log("=== End-to-End Nile Test Flow ===");
  ...
  console.log("\n[Step 1] Sending intent command...");
  const intentRes = await fetch(`${API_BASE}/api/intent`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ text: "Reserve 20 USDt for tickets" }),
  });
  if (!intentRes.ok) {
...
run().catch((err) => {
  console.error("\n[E2E ERROR] Flow failed:", err.message);
  process.exit(1);
});
